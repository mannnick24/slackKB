...
ragFilters) {
    return {
        ...
        function: {
            ...
            description: "Search the organisation's knowledge base for relevant information. Use this when the user asks a question that might be in the knowledge base. The server applies any active time/channel/user filters from the chat session to this search.",
            parameters: {
                type: "object",
                properties: {
        ...
        impl: async (args) => {
            ...
            try {
                ...
                const rows = await vectorRepo.searchChunks(orgId, embedding, embeddingCfg.dimensions, embeddingCfg.model, limit, ragFilters);
                logger.debug({
                    orgId,
